...
<template>
    <Swiper
      ...
      @swiper="onSwiper"
    >
      <SwiperSlide v-for="(slide, index) in swiperCard" :key="index"  @mouseenter="showSwiperCnt > 1 && onMouseEnter" @mouseleave="showSwiperCnt > 1 && onMouseLeave">
        <!-- {{slide}} -->
          <slot name="item"/>
      </SwiperSlide>
      <SwiperControls v-model:visualSwiper="visualSwiper" :class="showSwiperCnt < 2 ?'swiper-controls':''" class="row justify-end" />
    </Swiper>
</template>
