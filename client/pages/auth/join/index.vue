...
const join = async () => {
    // TODO send user Data to the join endpoint and redirect if  successful 
    console.log(123)
};
const findAddress = () => {
    addressSearch((data) => {
...
<template>
    ...
    <q-card class="my-16 max-w-sm w-600">
      <q-card-section class="join-ipt">
        ...
        <q-form @submit="join">
          <q-input v-model="user.email" class="mT10" :rules="[user_id_rules]" outlined label="이메일" autofocus  autocomplete="username"/>
          <q-input v-model="user.password" class="mT10" :rules="[user_pw_rules]" outlined label="비밀번호" type="password" autocomplete="current-password" @update:model-value="user.chkPassword = ''"/>
          <q-input v-model="user.chkPassword" class="mT10" :rules="[user_pw_check_rules]" outlined label="비밀번호 확인" type="password" autocomplete="current-password" />
          <q-input v-model="user.userName" class="mT10" :rules="[user_name_rules]" outlined label="이름" />
          <q-input v-model="user.phoneNumber" class="mT10" :rules="[user_phone_rules]" outlined label="연락처" />

          <div class="row justify-between items-stretch col-xs-12 mT10">
            <div class="col-6">
                <q-input v-model="user.postNum" readonly outlined label="우편번호" />
            </div>
          ...
          <div class="row justify-between items-start col-xs-12 mT10">
            ...
            <div class="col-6 detail-address">
                <q-input v-model="user.addr2" :rules="[user_detail_rules]" outlined label="상세주소" class="mL10" />
            </div>
          </div>
          <div class="btn-center">
            <q-btn class="sign-in" label="회원가입" type="submit" color="primary" />
          </div>
        </q-form>
      </q-card-section>
    </q-card>
...
.join-ipt{
  ...
  .detail-address {
    flex-grow: 1; /* 상세주소 필드가 가능한 한 많이 확장되도록 */
